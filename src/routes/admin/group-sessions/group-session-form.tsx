...
import { Link, LoaderFunctionArgs, useNavigate, useParams, useRouteLoaderData } from 'react-router-dom';
...
export const Component = () => {
	...
	const [formValues, setFormValues] = useState({
		...
		contactEmailAddress: '', // new -- COBALT ONLY
		recurrance: '' as 'single' | 'series', // new -- EXTERNAL ONLY
		...
		recurranceDescription: '', // new -- EXTERNAL ONLY
		imageUrl: '', // existing
	...
	const [showContactEmailInput, setShowContactEmailInput] = useState(
		formValues.facilitatorEmailAddress !== formValues.contactEmailAddress
	);
				...
				<Container>
					<GroupSessionFormSection
						...
						description={
							<>
								<p className="mb-4">
									Write a clear, brief title to help people quickly understand what your group session
									is about.
								</p>
								...
								<p className="mb-4">
									{!isExternal
										...
										: ''}
									Include a friendly URL to make the web address at {window.location.host} easier to
									read. A friendly URL includes 1-3 words separated by hyphens that describe the
									content of the webpage (ex. tolerating-uncertainty).
								</p>
							</>
						}
					>
					...
					<GroupSessionFormSection
						title="Location"
						description="Only virtual sessions are allowed at this time."
					>
					...
					<GroupSessionFormSection
						...
						description={
							<>
								...
								{!isExternal && (
									<p>
										Enter the information for the person who will be running this session. By
										default, the facilitator will receive an email whenever a user registers or
										cancels for the group session. You can choose to add a different email address
										to receive these notifications instead.
									</p>
								)}
							</>
						}
						...
						{!isExternal && (
							...
							>
								<InputHelper
									...
									required
									value={formValues.contactEmailAddress}
									onChange={({ currentTarget }) => {
										setFormValues((curr) => ({
											...curr,
											contactEmailAddress: currentTarget.value,
										}));
									}}
								/>
							</ToggledInput>
						...
						{isExternal ? (
							<>
								<ToggledInput
									...
									checked={formValues.recurrance === 'single'}
									onChange={() => {
										setFormValues((curr) => ({
											...curr,
											recurrance: 'single',
								...
								>
									...
									<InputHelper
										...
										value={formValues.recurranceDescription}
										onChange={({ currentTarget }) => {
											setFormValues((curr) => ({
												...curr,
												recurranceDescription: currentTarget.value,
											}));
										}}
										required
									/>
					...
					<GroupSessionFormSection
						...
						description={
							<>
								<p className="mb-4">
									Add an image that represents the subject matter of your group session. Choose one
									that looks good at different sizes — this image will appear across the Cobalt
					...
					<GroupSessionFormSection
						...
						description={
							<>
								<p className="mb-2">
									Visible sessions can be displayed in a collection with other similar group sessions.
									If no collection is selected, then the session will only be displayed in the
									Upcoming Sessions section.
								</p>

								<p>
									A hidden session will still be available on Cobalt, but users will need a direct
									link to access it.
								</p>
							</>
						}
